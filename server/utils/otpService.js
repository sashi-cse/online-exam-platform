...
const sendOtpNotification = async (identifier, rawOtp, isEmail) => {
  ...
  // 1. Email OTP Delivery (if SMTP credentials are present)
  ...
  // 2. SMS OTP Delivery via Twilio (if Twilio credentials are present)
  if (!isEmail && process.env.TWILIO_ACCOUNT_SID && process.env.TWILIO_AUTH_TOKEN) {
    try {
      ...
      console.warn(`⚠️ Twilio SMS delivery failed: ${err.message}`);
    }
  }

  // Fallback for live preview/testing when SMS/SMTP keys are not configured:
